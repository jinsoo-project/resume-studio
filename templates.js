  ...
  function axRuntime() {
    ...
    function home() {
      ...
      var stats = (D.heroStats || []).map(function (s, i) {
        ...
        return '<div><div class="ax-stat" data-raw="' + e(s.v) + '" style="font-size:clamp(42px,3.4vw,56px);font-weight:800;letter-spacing:-.03em;font-variant-numeric:tabular-nums;color:' + col + '">' + animNum(s.v) + '</div><div class="axmono" style="font-size:12px;letter-spacing:.1em;color:#8b91a7;margin-top:6px">' + e(s.k) + '</div></div>';
      }).join("");
      ...
      return '<header style="position:relative;padding:clamp(56px,8vw,96px) 0 72px;border-bottom:1px solid #e8eaf2">' +
        ...
        '<h1 style="margin:0 0 32px;font-size:clamp(44px,6.2vw,94px);line-height:1.08;font-weight:800;letter-spacing:-.035em">' + e(D.headline) + '<br><span id="ax-rot" class="axgrad" style="display:inline-block;transition:opacity .32s,transform .32s">' + e(rw) + '</span>로 증명하는<br>풀스택 마케터.</h1>' +
        '<p style="margin:0;font-size:clamp(17px,1.5vw,23px);line-height:1.7;color:#4b5268;max-width:680px">' + e(D.lede) + '</p>' +
